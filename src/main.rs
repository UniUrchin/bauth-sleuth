...
fn http_parser(timestamp: DateTime<Local>, payload: &[u8]) -> AResult<()> {
    ...
    let decrypted_bauth = decode(&encrypted_bauth)?.iter().map(|&s| s as char).collect::<String>();
    let ((username, _), password) = take_until_range(":").and(take(1)).parse(&*decrypted_bauth)?;
