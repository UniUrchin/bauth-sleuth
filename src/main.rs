...
fn main() {
    let network_interface = env::args()
        .nth(1)
        .expect("Error: Cannot find argument!!");
...
fn http_parser(timestamp: DateTime<Local>, payload: &[u8]) -> AResult<()> {
    ...
    for content in request_header {
        let ((header_name, _), value) = take_until_range(":")
            .and(take(2))
            .parse(content)?;
